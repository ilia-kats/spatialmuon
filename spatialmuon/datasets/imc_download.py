...
def create_muon_spatial_object(f_ome, f_masks, outfile):
    ...
    modality["FOV"] = fov = spatialmuon.Raster(X=np.moveaxis(ome.asarray(), 0, -1), channel_names = channel_names)
...
with tempfile.TemporaryDirectory() as tmpdir:
    ...
    download("https://zenodo.org/record/3518284/files/OMEandSingleCellMasks.zip?download=1", imgfile, desc="images")
    ...
    download("https://zenodo.org/record/3518284/files/SingleCell_and_Metadata.zip?download=1", metadatafile, desc="metadata")
    unzip(metadatafile, tmpdir, ["Data_publication/BaselTMA/Basel_PatientMetadata.csv", "Data_publication/ZurichTMA/Zuri_PatientMetadata.csv"])

    df = get_metadata(clean_verbose=True, basel_csv=os.path.join(tmpdir, "Data_publication", "BaselTMA", "Basel_PatientMetadata.csv"), zurich_csv=os.path.join(tmpdir, "Data_publication", "ZurichTMA", "Zuri_PatientMetadata.csv"))
